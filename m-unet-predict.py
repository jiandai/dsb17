...
feature_array = np.load('test_features-v2.npy')
print feature_array.shape
#feature_array_ = np.concatenate([np.load('test_features-v2-b'+str(j)+'.npy') for j in range(-1,1)])
#print feature_array.shape
...
# RF
pred(clf=rf,X=feature_array,y=truth_metric,csv=test_csv.copy(),outfile='stage1_submission_by_rf-v2.csv',validation=True)
pred(clf=rf,X=feature_array2,csv=stage2_csv.copy(),outfile='stage2_submission_by_rf-v2.csv')
# XG
pred(clf=xg,X=feature_array,y=truth_metric,csv=test_csv.copy(),outfile='stage1_submission_by_xg-v2.csv',validation=True)
pred(clf=xg,X=feature_array2,csv=stage2_csv.copy(),outfile='stage2_submission_by_xg-v2.csv')
