...
version 20170325 by jian: reuse the test script to understand segmentation
...
test_patient = patients[5]
from dicom_batch import get_one_scan
img_array = get_one_scan(INPUT_FOLDER+'/'+test_patient,resampling=False)
...
examples = [50,100,150]
n=len(examples)
fig,ax = plt.subplots(n,5,figsize=[8,8])
for j in range(n):
	ax[j,0].imshow(img_array[examples[j]],cmap='gray')
	ax[j,1].imshow(segmented_lungs[examples[j]],cmap='gray')
	ax[j,2].imshow(segmented_lungs_fill[examples[j]],cmap='gray')
	ax[j,3].imshow(img_array[examples[j]]*segmented_lungs[examples[j]],cmap='gray')
	ax[j,4].imshow(img_array[examples[j]]*segmented_lungs_fill[examples[j]],cmap='gray')
plt.show()
quit()
